...
The alpha diversity between genotypes was significantly different for all all genotypes and Hill orders, with the exception of the Hill order 2 (𝑞 = 2, *p* \> .05) comparison of F2WY and G genotypes (Fig. 2C).  The parental lines had significantly greater levels of Hill order 0 (𝑞 = 1, *p* \<.001, Fig. 2A) when compared to their F2 progeny across all habitats. We observed a similar pattern in beta diversity between habitats, all were significantly distinct from one another (Fig. 2D - 2F). Meadow 2 has consistently greater levels of beta diversity for all Hill orders, while Granite 1 has the lowest Hill order 0 (𝑞 = 0), but not Hill order 1 or 2 (Fig. 2D - 2F). 

The differences in FEF community composition were observed with a distance-based Redundancy Analysis (dbRDA) to model the relationship between FEF communities, leaf shape, and habitat type. The best fit model revealed that 15.72% of the overall variance in FEF communities was accounted for by log-transformed LBI, genotype, and habitat type, the constraining variables. The first axis (dbRDA1) explained 52.9% and the second axis (dbRDA2) explained 36.4% of the constrained variance (Fig. 4). We saw that Granite 1 and lobLBI, ar opposite to Madow 1 along dbRDA1.  When we visualize the plotting the ellipses for genotypes, we observed highly overlapping groupings of FEF communities in F2YW and F2Wy genotypes (Fig. 3A), as well less overlap in groupings of G and L genotypes. We note clear and little to non-overlapping groupings of FEF communities when results are visualized by habitat type (Fig. 3B). 

We found that FEF communities were homogeneous across for all genotypes (*F*~3,14096~= 46.28, *p* \=.001) by performing a permutational analysis of multivariate dispersion (PERMDISP) to test for homogeneity of variances. The post-hoc Tukey's test revealed that all genotype comparisons were significant different at ⍺ = 0.05. We also detected significant differences in FEF communities by habitat type (*F*~2,14097~= 2472.5, *p* \= .001). Our post-hoc Tukey's test showed that FEF communities from all genotypes and habitat types had significant differences in the dispersion of variance at ⍺ = 0.05.

To understand how leaf shape influences FEF communities, we measured LBI and observed significant differences in G and L genotypes  in Meadow 1(*p* \=.05, Fig. 4A), but not Meadow 2 and Granite 1. The F2WY and F2YW progeny only had significant differences when compared to each other, and F2WY when compared to G in Meadow 2. In Granite 1 we saw the F2 progeny has significant differences with parental lines (*p* \=.05, Fig 4A), but not when compare to each other (Fig. 4A). The LBI differences in LBI compared across habitat types were significant for the F2 progeny, parental lines did not differ significantly in LBI across habitats (Fig. 4B). 

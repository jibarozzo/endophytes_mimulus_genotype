...
In the rarified data set, comprised of 282 samples, we observed that 202 (38%) and 159 (30%) ASVs were shared across habitat types and cross-types, respectively (Fig. 2A - 2B). Within the ten most abundant genera, *Cladosporium*, *Vishiniacozyma*, and *Ramularia* are the most abundant across all samples (Fig. S2). 
...
Alpha diversity was significantly different between all parental lines and hybrid crosses, with the exception of the Hill order 2 (𝑞 = 2, *p* \> .05) comparison of F~2~LG and G parental line (Fig. 3C). The parental lines had significantly greater levels of observed richness (𝑞 = 0, *p* \< .001, Fig. 3A) when compared to their F~2~ progeny when accounting for all habitat types. Only F~2~GL had significant differences (*p* \< .001) in observed richness at all habitats. For F~2~GL and F~2~LG, we saw significant differences in Hill order 1 and 2 (𝑞 = 1, and 𝑞 = 2) when comparing Meadow 1 and Meadow 2 (*p* \< .001), and Granite 1 to Meadow 2 (*p* \< .001). Additionally, F~2~LG cross-type also had significant differences for these Hill orders when comparing Meadow 1 and Granite 1 (*p* \< .05). We observed a similar pattern in beta diversity, where habitats showed significant differences from one another (Fig. 3D - 3F). Meadow 2 had consistently greater levels of beta diversity for all Hill orders, while Granite 1 had the lowest observed richness (𝑞 = 0), but not Hill order 1 or 2 (Fig. 3D - 3F). Individuals from Granite 2 were not included in the analysis due 100% mortality after one week of planting at site (Fig. S4).

To further understand beta diversity and FEF community composition we performed two distance-based Redundancy Analyses (dbRDA) modeling the relationship between FEF communities, leaf shape, cross-type, and habitat type: the first accounting for individuals from parental lines and hybrids, and a second only with hybrids. The best fit model with all individuals and cross-types revealed that 18.71% of the overall variance in FEF communities was accounted for by log-transformed LBI, cross-type, and habitat type, the constraining variables. The first axis (dbRDA1) explained 45.3% and the second axis (dbRDA2) explained 31.5% of the constrained variance (Fig. 4). 
...
In contrast, we note clear and little to non-overlapping FEF communities when results are visualized by habitat type (Fig. 4B). The marginal significance of constraining variables after permutational ANOVA revealed that all variables were significant at ⍺ = 0.05 but varied in their explanatory power (Table 1). We saw evidence for strong habitat type effects on FEF communities. The dbRDA for hybrids revealed that 0.01% of the overall constrained variance was explained by log-transformed LBI, cross-type and habitat type. The constrained variance was explained by dbRDA axis 1 (65.5%) and axis2 (19.7%) (Fig. 5 and Table 2).

We performed permutational analysis of multivariate dispersion (PERMDISP) to test for homogeneity of variances of FEF communities from clusters in the dbRDA analyses. We found homogeneous variance of FEF communities across for all parental lines and crosses (*F*~3,~ ~14096~= 46.28, *p* \=.001). The post-hoc Tukey's test revealed that all parental lines and cross-type comparisons were significantly different at ⍺ = 0.05. We also detected significant differences in FEF communities by habitat type (*F*~2,~ ~14097~= 2472.5, *p* \= .001), as well as the interaction between habitat and cross types (*F*~2,~ ~14088~= 471.6, *p* \= .001 ). Our post-hoc Tukey's test showed that FEF communities from all habitat types had significant differences in the dispersion of variance at ⍺ = 0.05, meanwhile the interaction between habitat and cross-type showed more nuances for certain combinations, yet significant differences for most in the dispersion of variance at ⍺ = 0.05 (Table S4). For our model with only hybrid crosses, we found that homogeneity of variances of FEF communities were not significantly different between cross-types (*F*~1,~ ~11548~= 0.034, *p* \= .85). The post-hoc Tukey's test revealed that F~2~LG and F~2~GL had no differences in the dispersion of variance at ⍺ = 0.05.

Wilcoxon rank sum tests comparing log LBI means revealed no significant differences in parental lines and cross types across all habitat types at ⍺ = 0.05 (Fig. 6A). Meadow 1 and 2 did not differ significantly in LBI, while Granite 1 had greater LBI compared to Meadow 1 and 2. (*p* \<.001, Fig. 6B).
...
r1gg <- r1gg +
  ...
  theme(legend.title = element_text(face = "bold", size = 12),
         ...
         axis.text.x = element_blank())
r1gg
ggsave(file.path(path, "figures/figure1_rarefied_only.png") , plot = r1gg, dpi=300, units=c("mm"), width=300, height=300)
...
#| fig-width: 6.5
#| label: venn_diagrams
#| fig-cap: "Venn diagrams of presence and absence of ASVs among cross-types and habitat type in rarified data. A) Represents the overlap of ASVs habitat types and B) in genetic cross-types. Number of ASVs are represented as a count and in parentheses as a ratio shared between cross-type and habitat type."
...
# Save
ggsave(filename = file.path(path, "figures/figure2.png"), plot = venn_plots, dpi=300, units=c("mm"), width=250, height=225)
```
...
ggsave(filename = file.path(path, "figures/figure3.jpg"), plot = alpha_beta_diversity, dpi=300, units=c("mm"), width=300, height=275)
...
#Save the plot
ggsave(
  file.path(path, "figures/figure4.jpg"),
  plot = double_dbrda,
  dpi = 300,
  units = c("mm"),
  width = 250,
...
ggsave(
  filename = file.path(path, "figures/figure5.jpg"),
...
ggsave(
  filename = file.path(path, "figures/figure6.jpg"),
  plot = lbis,
  dpi = 300,
  units = c("mm"),
  width = 250,
...
#| eval: true
#| tidy: true
#| tbl-cap: "Permutational ANOVA for the best-fit dbRDA model with all cross-types. Significance levels are represented by asterisks [*p* < .05 (\\*), *p* < .01 (\\*\\*), *p* < .001 (\\*\\*\\*), and *p* < .0001 (\\*\\*\\*\\*)]."
...
# Table
table1 <- flextable(anova_table1) |>
  flextable::set_table_properties(width = .5) |>
  flextable::set_table_properties(layout = "autofit") |>
  flextable::style(i = c(1:5),
                   j = 1,
  ...
  flextable::set_caption("Permutational ANOVA for the best-fit dbRDA model with all cross-types.")
...
#| eval: true
#| tidy: true
#| tbl-cap: "Permutational ANOVA for the best-fit dbRDA model with hybrid cross-types. Significance levels are represented by asterisks [*p* < .05 (\\*), *p* < .01 (\\*\\*), *p* < .001 (\\*\\*\\*), and *p* < .0001 (\\*\\*\\*\\*)]."

#dbRDA table  (results from terms are the same as adonis2 but without R2)
anova_m2_hybrids_margin <- readRDS("clean_data/statistics/anova_m2_hybrids_margin.rds")
anova_table2 <- tidy(anova_m2_hybrids_margin) |>
  as.data.frame() |>
  mutate("p-value" = str_replace_all(p.value, "0.001", "***")) |>
  rename(
    Term = term,
...
table2 <- flextable(anova_table2) |>
  ...
  flextable::set_caption("Permutationa ANOVA for the best-fit dbRDA model with hybrid cross-types. Significance levels are represented by asterisks")
...
::: {#suppfig-1}
![Greenhouse cross-type design and reciprocal transplant design. Greenhouse hybrid crosses from parental lines in dashed line box. Reciprocal transplant design at each site in solid line boxes and ovals. Red squares represent F~2~GL hybrid cross. Light blue represents F~2~LG hybrid cross. Yellow represents *M. laciniatus* parental line, and orange represents *M. guttatus* parental line.](figures/supp_fig1_diagram.png){width="4.5in"}
:::
...
#| fig-height: 6
#| label: suppfig-2
#| fig-cap: "Heatmap of the top 10 most abundant FEF genera in the dataset. The heatmap is faceted by habitat type and cross-type."
...
# Save the plot
ggplot2::ggsave(filename = file.path(path, "figures/supp_fig2.jpg"), plot = r2gg, dpi=300, units=c("mm"), width=300, height=250)
```
...
#| fig-height: 6
#| label: suppfig-3
#| fig-cap: "Alpha diversity mean comparisons in cross-types facetted by habitat types. Alpha diversity means comparisons: A) Observed ASV richness (𝑞 = 0); B) Shannon's entropy (𝑞 = 1); and C) Inverse Simpson's index (𝑞 = 2) per host species. A-C) Red filled boxplots correspond to F~2~GL, orange filled to G (*M. guttatus*), yellow to L (*M. laciniatus*), and teal to F~2~LG cross-type. Beta diversity mean comparisons; A) Observed ASV richness (𝑞 = 0); B) Shannon's entropy (𝑞 = 1); and C) Inverse Simpson's index (𝑞 = 2)per elevation category. D-F). Significance levels are represented by *ns* (not significant) and asterisks [*p* < .05 (\\*), *p* < .01 (\\*\\*), *p* < .001 (\\*\\*\\*), and *p* < .0001 (\\*\\*\\*\\*)]."
...
# Joined plots
supp_fig3 <- ggarrange(hill0_species,"", hill1_species,"", hill2_species,
                                  nrow = 5, ncol = 1,
...
ggsave(filename = file.path(path, "figures/supp_fig3.jpg"), plot = supp_fig3, dpi=300, units=c("mm"), width=275, height=300)
...
#| fig-height: 6
#| label: suppfig-4
#| fig-cap: "Map of reciprocal transplant sites in Yosemite National Park. Sampling sites are color-coded by habitat type. Granite 1 (green), Meadow 1 (purple), and Meadow 2 (teal). Granite 2 (open-crossed circle) was not sampled."
...
```{r, supp_table}
supp_table2 <- phyloseq_ntaxa_by_tax(
  ps_clean_3,
  TaxRank = "Phylum",
  relative = F,
  add_meta_data = T
  ...
  summarise("Ocurrence in samples" = n()) |>
...
### Custom function for rarefying data

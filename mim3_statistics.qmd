...
anova_m2_hell_terms <- readRDS("clean_data/statistics/anova_m2_hell_terms.rds")
...
```
The groups interacting with the environment (Habitat_Genotype) show significant differences in multivariate dispersion (p \< 0.05). The PERMDISP analysis shows that the groups are significantly different in their multivariate dispersion.Group differences are due to dispersion differences between groups, not due to differences in the centroid of the groups.
...
# Rearranging the order of layers so arrows are above the points.
# full_final_model$layers <- c(full_final_model$layers,full_final_model$layers[[2]], full_final_model$layers[[5]], full_final_model$layers[[6]], full_final_model$layers[[7]], full_final_model$layers[[8]], full_final_model$layers[[9]], full_final_model$layers[[10]], full_final_model$layers[[11]])
full_final_model
...
hybrid_final_model$layers <- c(
  hybrid_final_model$layers,
  hybrid_final_model$layers[[2]],
  hybrid_final_model$layers[[5]],
  hybrid_final_model$layers[[6]],
  hybrid_final_model$layers[[7]],
  hybrid_final_model$layers[[8]],
  hybrid_final_model$layers[[9]],
  hybrid_final_model$layers[[10]]
)
hybrid_final_model
...
# Model with intercept only
B0 <- lm(mean_bray ~ 1, data = dbrda_traits_imp, na.action = na.exclude)
anova(B0)
AIC(B0)
plot(B0)
...
################################################
# B1 Model with Elevation, Species and Site
################################################
B1 <- lm(
  mean_bray ~ Elevation_m + Species + Site + logACI + logLT + logLPS + logLMA + logLBI,
...
################################################
# B2 Model with Species and Site
################################################
B2 <- lm(
  mean_bray ~ Species + Site + logACI + logLT + logLPS + logLMA + logLBI,
  data = dbrda_traits_imp,
...
B1 and B2 models have the same AIC score but they do not have the same fixed terms. I believe this is due to the minimal effect that Elevation has on the model (Estimates).
...
#B3
B3 <- lme(
  mean_bray ~ Species + logACI + logLT + logLPS + logLMA + logLBI,
  random = ~ 1 | Site,
  method = "ML",
  data = na.omit(dbrda_traits_imp)) 

Anova(B3, type = 3)
AIC(B3) #  -13522.89 Increased AIC score compared to B1 and B2.
...
The GLMMs with "ML" methods and "Site" as a random effects has a greater AIC value than B1 model. The stepwise selection eliminated "Elevation" as a fixed term in the model. The final model is `mean_bray ~ Species + Site`. There could be some variance structure that needs to be modeled. We will still keep "Elevation" while we focus in nestedness in the variance structure. Let's see. 

```{r, random_effects2}
# B5 
B4 <- lme(
  mean_bray ~ logACI + logLT + logLPS + logLMA + logLBI,
  random = ~ 1 |
    Site / Species,
  method = "ML",
  data = na.omit(dbrda_traits_imp)
...
Anova(B4, type = 3)
AIC(B4) # -13717.34 is a higher AIC score than B3.
...
# logLT and logLBI eliminated from the model.
```
The B4AIC model has a lower AIC score. We seem to have hit a good model. The fixed terms are `mean_bray ~ logACI + logLPS + logLMA`. The delta AIC is ~2.5. I say we keep the terms since it is not a huge improvement and model random effects. Let's take that an model random effects taking into account variance structure. 

I will keep `Species` as a fixed term. Only `Site` as random effect. Maybe `Sampling_date` could be a random effect.
...
B1REM <- lme(
  mean_bray ~ Elevation_m +  Species + logACI + logLT + logLPS + logLMA + logLBI,
  random = ~ 1 |
    Site,
  method = "REML",
  data = na.omit(dbrda_traits_imp))
...
Anova(B1REM, type = 3)
AIC(B1REM) # -13437.7 This is a higher score than B4AIC.
...
#Anova(B1_gls, type = 3)
AIC(B1_gls) # -11858 is a higher AIC score. Site as a random effect is important.

plot_model(B1REM, type = "diag")

# Model comparison
model.sel(B1, B2, B3, B4, B4AIC, B1REM, B1_gls)[, c("df", "logLik", "AICc", "delta")]

# Model B1 and B2 are the best models. B1 has a lower AIC score.
```

The REML model with Elevation and Species as fixed terms has a higher AIC score than the `gls` model without random effects. Site as a random effect is important. Now, playing around with the method, I observed that "ML" models the data better than "REML". The AIC scores are lower.
...
# Stepwise selection of terms
#drop1(B3REM, . ~ ., test = "Chisq", trace = TRUE)
```

The B3REM model has the lowest AIC score yet. The the fixed terms are `mean_bray ~ Elvation + Species + logACI + logLT + logLPS + logLMA + logLBI,` with a variance structure `weights = varIdent(form = ~ 1 | Site)`. The random effects are nested `random = 1 | Site`. It seems to be a good fit for the data. Now I eliminate `Elevation` and model with the remaining fixed terms ` Species + logACI + logLT + logLPS + logLMA + logLBI`.
...
anova(B4REM) # Adding Species as a random effect DECREASES AIC score.
Anova(B4REM, type = 3)
AIC(B4REM) # -10690.23 the lowest AIC score
...
save(B1, B2, B3, B4, B4AIC, B1REM, B3REM, B4REM, B5REM, file = "clean_data/statistics/glmm_models.rda")

load("clean_data/statistics/glmm_models.rda")

#Model comparison and selection
model.sel(B1, B2, B3, B1REM, B3REM, B4AIC, B4REM, B5REM)[, c("df", "logLik", "AICc", "delta")]
```
...
model_tables <- tab_model(
  ...
  show.df = FALSE,
  show.ci = 0.95,
  title = "Linear mixed effects models for predicting FEF mean Beta diversity",
  CSS = list(

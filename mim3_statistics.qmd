...
plant_traits_rfy <- plant_traits_rfy |>
  select(!c(10:11)) |> # Removing variables calculated before rarefactions
  mutate(X = Unique_ID,
...
dbrda_hybrids_matrix <- rrfy_hell_matrix |>
  ...
  t() |>
  as.data.frame()
saveRDS(dbrda_hybrids_matrix, file = file.path(path, "clean_data/statistics/dbrda_hybrids_matrix.rds"))

dbrda_hybrids_traits <- rrfy_hell_matrix |>
  select(X, Unique_ID, Site, Genotype, logLBI)|>
  filter(X %in% rownames(dbrda_hybrids_matrix))
saveRDS(dbrda_hybrids_traits, file = file.path(path, "clean_data/statistics/dbrda_hybrids_traits.rds"))
...
```{r, final_model}
# Final model
m2_hell <- load("clean_data/statistics/m2_hell.rda")
final_model <- m2_hell
...
set.seed(123)
beta_perm1 <- permutest(beta_dis1, parallel = 8, permutations = 999, by = "margin")
...
# By Elevation
# set.seed(123)
# beta_dis2 <- betadisper(vegdist(dbrda_hell_matrix, method = "bray"), 
...
set.seed(123)
beta_perm2 <- permutest(beta_dis2, parallel = 8, permutations = 999, by = "margin")

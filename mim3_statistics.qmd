...
# Leaf level data set
leaf_traits <-  left_join(traits, select(imagej, Unique_ID, ImageJ_1, ImageJ_2, Leaf_Lobe_Index), join_by(Unique_ID)) |>
  select(-Notes) |>
  mutate(logLBI = log10(Leaf_Lobe_Index),
         logACI = log10(ACI),
...
plant_traits <- leaf_traits |>
  ...
  group_by(Unique_ID, Site, Block, Genotype)|>
...
##### Rarefied data frame
Function alpha_rfy_DF() return a data frame with rarefied alpha diversity, BUT also rarefied leaf traits. Somehow it randomizes values and we end up with CF_5_4_F2WY matching with genotype G and corresponding data. Here I remove the randomized leaf trait data and assign the correct values to each sample. It needs to be the same for Unique_ID accrose rarefied data frames.
```{r, rarefaction_output}
plant_traits_rfy <- plant_traits_rfy |>
  select(c(16:20)) |> # Removing variables calculated before rarefactions
  mutate(X = Unique_ID,
  ...
  left_join(plant_traits, by = "Unique_ID") |> # Joining with plant_traits data frame
...
rrfy_hell_matrix <- as.data.frame(rrfy_hell_matrix) |>
  ...
  relocate(Unique_ID, .after = X) |>
  left_join(plant_traits, by = "Unique_ID") |>
  relocate(c(535:549), .before = ASV_1)
...
full_final_model <- ggplot(data = merged_data, aes(x = dbRDA1, y = dbRDA2)) +
  geom_point(data = merged_data, aes(color = Genotype, shape= Site), size = rel(4), alpha = 0.7, stroke = 1) +
  ...
  geom_text(label_map, size = rel(4), data = A4[[1]], fontface = "bold", show.legend = FALSE, label = c("F2WY")) +
  geom_text(label_map, size = rel(4), data = A4[[2]], fontface = "bold", show.legend = FALSE, label = c("G")) +
  geom_text(label_map, size = rel(4), data = A4[[3]], fontface = "bold", show.legend = FALSE, label = c("L")) +
  geom_text(label_map, size = rel(4), data = A4[[4]], fontface = "bold", show.legend = FALSE, label = c("logLBI")) +
  geom_text(label_map, size = rel(4), data = A4[[5]], fontface = "bold", show.legend = FALSE, label = c("LM")) +
  geom_text(label_map, size = rel(4), data = A4[[6]], fontface = "bold", show.legend = FALSE, label = c("OPN")) +
...
# full_final_model$layers <- c(full_final_model$layers, full_final_model$layers[[5]], full_final_model$layers[[6]])
full_final_model

ggsave(filename=file.path(path, "figures/dbRDA_species.png"), plot = full_final_model, dpi=600, units=c("mm"), width=150, height=150)
```
...
merged_data <- right_join(ordination_scores, dbrda_df, by = c("Unique_ID" = "X"))
  select(!Unique_ID.y)
  filter(Genotype == "F2WY" | Genotype == "F2YW")
...
hybrid_final_model <- ggplot(data = merged_data, aes(x = dbRDA1, y = dbRDA2)) +
  geom_point(data = merged_data, aes(color = Site, shape = Genotype), size = rel(4), alpha = 0.7, stroke = 1) +
  geom_segment(arrow_map, size = rel(.8), data = arrow_scores, color = "black", arrow = arrowhead) +
  geom_vline(xintercept = 0, color = "grey30", linetype = "dashed") + # plot vertical line
  geom_hline(yintercept = 0, color = "grey30", linetype = "dashed") + # plot horizontal line
  # geom_text(label_map, size = rel(4), data = A4[[1]], fontface = "bold", show.legend = FALSE, label = c("F2WY")) +
...
# hybrid_final_model$layers <- c(hybrid_final_model$layers, hybrid_final_model$layers[[5]], hybrid_final_model$layers[[6]])
hybrid_final_model
